...
public class Barrack extends Unit {
	...
	//To count units
	public Map<String,String> BarrackMap = new HashMap<String,String>(20);
	
	public Barrack(String civName, InputStream in, String tag) {
		// TODO Auto-generated constructor stub
		barrackTag=tag;
		retrieve(civName, in, tag);
		
		name = XMLfetch.get("name");
		Log.i("null var", (XMLfetch.get("id"))+" "+barrackTag);
	...
	public void retrieve(String civName, InputStream in, String tag) {
		...
		try {
			...
			Log.i("team1", "Inside startElement "+barrackTag);
		
			if(qName.equalsIgnoreCase("building"))
			{
				Log.i("team1", "inside EqualsIgnoreCase(buildG) " +barrackTag);
				inBuilding = true;
				building = true;
				...
				if(id==0)
				{
					Log.i("team1", "Inside if(id==5) " +barrackTag);
			...
			Log.i("team1", "before going to cost node "+barrackTag);
			if(inBuilding && id==0){
				Log.i("team1", "before going to cost node");
				if(qName.equalsIgnoreCase("cost")){

					Log.i("team1", "Inside COST node "+barrackTag);
					Log.i("resources", "Wood "+attributes.getValue("wood")+barrackTag);
					Log.i("resources", "Stone "+attributes.getValue("stone")+barrackTag);
					Log.i("resources", "Food "+ attributes.getValue("food")+barrackTag);
					Log.i("resources", "Gold "+attributes.getValue("gold")+barrackTag);
				...
				if(qName.equalsIgnoreCase("unit")){
					...
					BarrackMap.put(attributes.getValue("name"),"0");
