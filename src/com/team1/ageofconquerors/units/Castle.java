...
public class Castle extends Unit {
	...
	//To count units
	public Map<String,String> CastleMap = new HashMap<String,String>(20);
	
	public Castle(String civName, InputStream in, String tag) {
		// TODO Auto-generated constructor stub
		castleTag=tag;
		retrieve(civName, in, tag);
		
		name = XMLfetch.get("name");
		Log.i("null var", (XMLfetch.get("id"))+" "+castleTag);
	...
	public void retrieve(String civName, InputStream in, String tag) {
		...
		try {
			...
			Log.i("team1", "Inside startElement "+castleTag);
		
			if(qName.equalsIgnoreCase("building"))
			{
				Log.i("team1", "inside EqualsIgnoreCase(buildG) " +castleTag);
				inBuilding = true;
				building = true;
				...
				if(id==9)
				{
					Log.i("team1", "Inside if(id==5) " +castleTag);
			...
			Log.i("team1", "before going to cost node "+castleTag);
			if(inBuilding && id==9){
				Log.i("team1", "before going to cost node");
				if(qName.equalsIgnoreCase("cost")){

					Log.i("team1", "Inside COST node "+castleTag);
					Log.i("resources", "Wood "+attributes.getValue("wood")+castleTag);
					Log.i("resources", "Stone "+attributes.getValue("stone")+castleTag);
					Log.i("resources", "Food "+ attributes.getValue("food")+castleTag);
					Log.i("resources", "Gold "+attributes.getValue("gold")+castleTag);
				...
				if(qName.equalsIgnoreCase("unit")){
					...
					CastleMap.put(attributes.getValue("name"),"0");
